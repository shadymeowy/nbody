...
Vec3 calculateForce(int body_idx, const std::vector<Body>& bodies, Octan* octan, double theta) {
    ...
    // we got the corresponding body!
    const Body& body = bodies[body_idx];

    // distance between the body and COM!
    // will be used at ((xcom-x)/r^3, (ycom-y)/r^3, (zcom-z)/r^3)
    // where r will be r = sqrt((xcom-x)^2 + (ycom-y)^2 + (zcom-z)^2)
    ...
    double d = distance;
    // if the s/d ratio is less than theta, which means the points are far enough to treat as one point!
    if (s/d < theta) {
        double F = G * body.mass * octan->body_mass/squaresum;
        force.v[0] = F*dx/distance;
        force.v[1] = F*dy/distance;
